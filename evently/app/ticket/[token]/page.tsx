...
export default async function TicketPage({
  ...
  const qr = await QRCode.toDataURL(
    `${process.env.NEXT_PUBLIC_APP_URL}/ticket/${token}`,
    {
      width: 700,
      margin: 2,
      ...
      <div className="mx-auto max-w-md">
        ...
        <div className="ticket-notch mt-5 overflow-hidden rounded-[30px] bg-bone text-ink-950 shadow-premium">
          <div className="bg-brass-gradient p-7 text-ink-950">
            <div className="text-xs font-semibold tracking-[.25em] text-ink-950/50">
              DIGITAL TICKET
            </div>
          ...
          <div className="p-7">
            {/* Only show this banner after the ticket has been scanned. */}
            {isUsed && (
            ...
            <p className="text-xs font-semibold tracking-widest text-ink-950/40">
              ATTENDEE
            </p>
        ...
        <p className="mt-6 text-center text-xs leading-5 text-bone/30">
          Present this QR code at the event entrance. Each ticket can be used
          once.
        </p>
      </div>
    </main>
